 ...
 */
?>
<div class="group flex items-center px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg hover:bg-bg-card transition-all cursor-pointer" 
     data-song-id="<?= e($track->id) ?>"
     onclick="Player.play(<?= e($track->id) ?>)">
    
    <!-- Index or Play button -->
    <?php if (isset($showIndex) && $showIndex): ?>
        <div class="w-6 sm:w-8 mr-3 sm:mr-4 text-center flex-shrink-0">
    ...
    <!-- Cover - Petit carré avec angles arrondis -->
    <img 
        src="<?= e($track->cover_path ?? asset('images/default-cover.svg')) ?>" 
        alt="<?= e($track->title) ?>"
        class="w-10 h-10 sm:w-11 sm:h-11 rounded-md object-cover mr-3 sm:mr-4 flex-shrink-0 shadow-sm"
    >
